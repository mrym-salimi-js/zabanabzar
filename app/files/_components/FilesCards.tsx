...
// Card
export function FileCard({ filesList }: TableRowProps) {
  return filesList?.map((file) => {
    return (
      <div
        key={file.id}
        className="w-full h-auto flex flex-col p-2 gap-1.5 border-2 border-gray-300 rounded-2xl overflow-hidden bg-white dark:bg-[var(--background-dark)] "
      >
        {/* Header */}
        <div className="w-full h-10 flex p-2 items-center justify-between dark:bg-[var(--tertiary-dark)] border-b-2 rounded-md">
          <CardMoreActions />
          <GreenCheckBox />
        </div>
        ...
        <div className="w-full h-auto p-2 flex flex-row-reverse items-center justify-between">
          <div className="w-auto flex flex-row-reverse items-center gap-2  dark:text-white">
            ...
            <div className="w-auto flex flex-col items-end">
              {/*File name */}
              <p className="max-w-[80%] truncate text-[0.8rem] ">{file.name}</p>
              {/* Size */}
          ...
          <div className="flex gap-1 items-center">
            {/* Delete */}
            <span className="w-auto h-auto p-2 rounded-full cursor-pointer hover:opacity-[0.7] bg-[var(--primary-light)] dark:bg-[var(--primary-dark)]">
        ...
        <div className="flex flex-col">
          ...
          <div className="w-full h-auto  p-2 flex  items-center justify-between">
            <div className=" text-start flex text-[0.8rem] text-gray-400 dark:text-[var(--tertiary)] items-center gap-1 min-w-0 ">
              <p>
                {toPersianNumbers(
                  moment(file.createdAt).format("jYYYY/jMM/jDD")
                )}
              </p>
              <p>{" | "}</p>
              <div className="flex gap-1 items-center ">
          ...
          <div className="w-full h-auto  p-2 flex  items-center justify-between">
            <div className=" text-start flex text-[0.8rem] text-gray-400 items-center gap-1 min-w-0 dark:text-[var(--tertiary)]">
              <p>
                {toPersianNumbers(
                  moment(file.updatedAt).format("jYYYY/jMM/jDD")
                )}
              </p>
              <p>{" | "}</p>
              <div className="flex gap-1 items-center ">
