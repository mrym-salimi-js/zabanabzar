...
export function FileCard({ filesList }: TableRowProps) {
  return filesList?.map((file) => {
    return (
      <>
        <div className="w-full h-auto flex flex-col p-2 gap-1.5 border-2 border-gray-300 rounded-2xl overflow-hidden ">
          {/* Header */}
          <div className="w-full h-10 flex p-2 items-center justify-between dark:bg-[var(--tertiary-dark)] border-b-2">
            <CardMoreActions />
            <GreenCheckBox />
          </div>
          ...
          <div className="w-full h-auto p-2 flex flex-row-reverse items-center justify-between">
            <div className="w-auto flex flex-row-reverse items-center gap-2  dark:text-white">
              ...
              <div className="w-auto flex flex-col items-end">
                {/*File name */}
                <p className="max-w-[80%] truncate text-[0.8rem] ">
                  {file.name}
                </p>
                {/* Size */}
